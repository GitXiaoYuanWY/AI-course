...
class SectionScroller {
    ...
    updateNavigation(index) {
        ...
        // section 0 是 Hero，不激活任何导航
        if (numericIndex === 0) {
            activeMainNavIndex = -1;
        } else if (numericIndex >= 1 && numericIndex <= 4) {
            activeMainNavIndex = numericIndex - 1; // 减1因为删除了"课程概览"
        } else if (numericIndex === 5) {
            activeMainNavIndex = 4; // 五、RTCF要素
        } else if (numericIndex >= 6 && numericIndex <= 9) {
            activeMainNavIndex = 4; // 仍然激活"五、RTCF要素"
        } else if (numericIndex === 10) {
            activeMainNavIndex = 9; // 六、优化规范
        } else if (numericIndex === 11) {
            activeMainNavIndex = 10; // 七、课程总结
        }
...
function triggerPageAnimation(pageName) {
    const animationMap = {
        'overview': () => {
            ...
            if (h1 && subtitle) {
                typeWriter(h1, '精准控制AI输出的提示词用法', 100);
                typeWriter(subtitle, '掌握提示词技巧，让AI真正听话', 50);
            }
        },
        'course': () => {
            ...
            if (h1 && subtitle) {
                ...
                typeWriter(subtitle, '深入学习提示词的核心技巧', 50);
            }
        },
        'practice': () => {
            ...
            if (h1 && subtitle) {
                typeWriter(h1, '📝 实战案例', 100);
                typeWriter(subtitle, '小红书风格新品饮品文案示例', 50);
            }
        }
    };
